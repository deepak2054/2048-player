...
from core.gui import BACKGROUND_columnOR_DICT
import columnorlover as cl
import math
...
def test_generator(n):
    ...
    for i in range(n):
        ...
        temp["max_tile"] = random.choice(list(BACKGROUND_columnOR_DICT.keys()))
        lst.append(temp)
    return lst
...
class Visualizer:
    ...
    def draw_stack_bar(self, key):
        count_matrix = []
        data_of_key, labels = self.get_value_by_key(key)
        all_uniques_cells = sorted(list(BACKGROUND_columnOR_DICT.keys()))
        for i in range(len(data_of_key)):
        ...
        for i in range(len(np_matrix)):
            trace0 = go.Bar(
                ...
                marker = dict(
                    # cmax=len(np_matrix),
                    # cmin=0,
                    # columnorscale='Viridis',
                    columnor = bupux[gap * i]
                )
            )
